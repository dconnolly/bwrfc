<div class="module new-players">
    <div class="module-header">
        <h2>Play for Boston</h2>
    </div>
    <div class="module-body">
       <div><h3>Nationally ranked D1 club rugby</h3></div>
       <a href="/cape-cod-sevens">
           <img src="<?php echo get_stylesheet_directory_uri(); ?>/img/new-players-ruck-warmup.jpg" alt="Play for BWRFC, ruck with the best">
       </a>
    </div>
    <div class="module-footer">
        <h6><a href="/team/new-players"><i data-icon="&#xe13c;"></i>Get in touch</a></h6>
    </div>
</div>

<div class="module cape-cod-sevens">
    <div class="module-header">
        <h2>Cape Cod Sevens</h2>
    </div>
    <div class="module-body">
       <div><h3>Disney dreams come alive on July 13th</h3></div>
       <a href="/cape-cod-sevens">
           <img src="<?php echo get_stylesheet_directory_uri(); ?>/img/cape-cod-sevens-tutus.jpg" alt="Cape Cod Sevens, with tutus">
       </a>
    </div>
    <div class="module-footer">
        <h6><a href="/cape-cod-sevens"><i data-icon="&#xe13c;"></i>Prepare for battle</a></h6>
    </div>
</div>

